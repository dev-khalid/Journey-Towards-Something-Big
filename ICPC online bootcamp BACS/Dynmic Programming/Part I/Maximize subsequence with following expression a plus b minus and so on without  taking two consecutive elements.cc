...
public:
    int dp[100000][2][2];
    int helperFunc(vector<int> nums,int p1,int p2,int prev)
    {
        if(p1>=nums.size())
        {
            return 0;
        ...
        else
        {
            if(prev)
            {
                ...
                int val2 = helperFunc(nums,p1+1,p2,prev);
                return dp[p1][p2][prev] = val2;
            }
            else
            {

                //taking the current element
                int val1 = helperFunc(nums,p1+1,!p2,!prev) + (p2?-1*nums[p1]:nums[p1]);
                //skipping the current element
                int val2 = helperFunc(nums,p1+1,p2,prev);
                return dp[p1][p2][prev] = max(val1,val2);
            }
        }
    }
    int maxSequence(vector<int> &nums)
    {
        ...
        helperFunc(nums,0,0,0);
        return dp[0][0][0];
    }

};
//could have used pos+2 also ;
class Solution2
{
...
public:
    ...
    int helperFunc(vector<int> nums,int p1,int p2)
    {
        ...
        {
            //taking the current element
            int val1 = helperFunc(nums,p1+2,!p2) + (p2?-1*nums[p1]:nums[p1]);
            //skipping the current element
